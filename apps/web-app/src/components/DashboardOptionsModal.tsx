...
export function DashboardOptionsModal() {
  ...
  const uniqueStatuses = [...new Set(tasks.map((task: any) => task.status || 'Unknown'))];
  ...
  const completedStoryPoints = tasks.filter(t => t.status?.name === "done").reduce((acc, t) => acc + (t.storyPoints || 0), 0);
